...
"""
USE : like any other regular python file but in FreeCAD --hidden --console

"""

sys.argv.append("--hidden")
#FreeCAD.setConstructorOptions("NoGui")
...
doc = App.newDocument("simulation_mecanique")
App.setActiveDocument("simulation_mecanique")
...
def create_3D_model(file_name, infill_height=20, bottom_height=0, top_height=0,  extrusion_thickness=0.5):
    ...
    """
    doc = App.getDocument("simulation_mecanique")
...
def add_simulation():
    ...
    #analysis.Document.ActiveObject = analysis
...
def compute_maillage(piece, analysis):
...
def extract_deplacement(analysis):
    resultat = None
...
def run():
    ...
    compute_maillage(model, analysis)

    traction_force = add_constraints(model, analysis,  1)

    fea = add_solveur(analysis)
    ...
    for c in l_charge:
        ...
        run_solveur(fea)
        l_deplacement_x.append(extract_deplacement(analysis)["x"])


    print(l_charge)
    print(l_deplacement_x)
